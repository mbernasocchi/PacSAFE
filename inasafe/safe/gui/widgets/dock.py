...
class Dock(QtGui.QDockWidget, FORM_CLASS):
    ...
    def set_dock_title(self):
        ...
        self.setWindowTitle(self.tr('PacSafe %s' % self.inasafe_version))
    ...
    def accept(self):
        ...
        try:
            ...
            if (Config.get("settings", "multi") == "True"):
                ...
                for x in tl:                    
                    self.impact_function_parameters['threshold'] = x
                ...
                rno_no_impact =  sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"safe_ag" = 0')))
                
                rno_low_impact =  sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"safe_ag" = 1')))
                
                layer = registry.mapLayersByName("Medium Estimated Impact")[0] 
                rno_medium_impact =  sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"safe_ag" = 1')))

                layer = registry.mapLayersByName("High Estimated Impact")[0] 
                rno_high_impact = sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"safe_ag" = 1')))     

                layer = registry.mapLayersByName("Extreme Estimated Impact")[0]  
                rno_extreme_impact = sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"safe_ag" = 1')))         
