...
class Dock(QtGui.QDockWidget, FORM_CLASS):
    ...
    def set_dock_title(self):
        ...
        version = get_version()
        self.setWindowTitle(self.tr('PacSafe %s' % version))
    ...
    def getting_started_message(self):
        ...
        message.add(m.Heading(self.tr('Getting Started With PacSafe'), **INFO_STYLE))
        notes = m.Paragraph(
            self.tr('These are the minimum steps you need to follow in order to use PacSAFE. Click on the PacSAFE Icon'),
            m.Image('file:///%s/img/icons/spcicon.png' % (resources_path()), **SMALL_ICON_STYLE),
            self.tr('in the toolbar to load prepared hazard and exposure layers. To load and act on your own datasets, follow the steps below:')
            ) 
        message.add(notes)
        ...
        basics_list.add(m.Paragraph(
            ...
            self.tr(' layer (e.g. flood depths) to QGIS.')))
        basics_list.add(m.Paragraph(
            self.tr('Add at least one '),
            m.ImportantText(self.tr('exposure'), **KEYWORD_STYLE),
            self.tr(' layer (e.g. PCRAFI building data) to QGIS.')))
        basics_list.add(m.Paragraph(
            self.tr(
    ...
    def accept(self):
        ...
        try:
            ...
            # Start the analysis
            #QMessageBox.information(self, "Debug", str(self.impact_function_parameters))
            print self.impact_function_parameters
            #QMessageBox.information(self, "Debug", str(type(self.impact_function_parameters)))

            #OrderedDict([('threshold [m]', 1.0), ('postprocessors', OrderedDict([('BuildingType', [<safe_extras.parameters.boolean_parameter.BooleanParameter object at 0x7f58718948d0>])]))])
            #verify file
            path = tempfile.tempdir + "/pacsafe.ini"
            if (os.path.exists(path) == False):
                QMessageBox.warning(self, "PacSAFE", "Options are not defined! Please setup options before generating impact map.")
                return
            
            Config = ConfigParser.ConfigParser()            
            Config.read(path)
            #QMessageBox.information(self, "Debug", "MultiRun: " + str(type(Config.get("settings", "multi"))))
            
            if (Config.get("settings", "multi") == "True"):
                QApplication.setOverrideCursor(QCursor(Qt.WaitCursor))
                #QMessageBox.information(self, "Debug", "MultiRun: " + str(Config.get("settings", "multi")))
                layerLabels = ["Low", "Medium", "High", "Extreme"]
                layerLabelCount = 0
                registry = QgsMapLayerRegistry.instance()
                layers = registry.mapLayers()
                legend = self.iface.legendInterface()
                tl = [float(Config.get("settings", "low")), float(Config.get("settings", "med")), float(Config.get("settings", "high")), float(Config.get("settings", "ext"))]
                for x in tl:
                    self.impact_function_parameters['threshold [m]'] = x
                    self.analysis.run_analysis()
                    #rename Estimated layers low, med, high, extreme
                    layer = registry.mapLayersByName("Estimated buildings affected")[0]
                    layer.setLayerName(layerLabels[layerLabelCount] + " Estimated Impact")                     
                    #add field "IMPACT" to layer with value layerLabels[layerLabelCount]
                    vpr = layer.dataProvider()
                    vpr.addAttributes([QgsField("IMPACT", QVariant.String)])
                    layer.updateFields()
                    #add value for IMPACT
                    it = layer.getFeatures()
                    fid = layer.fieldNameIndex("IMPACT")
                    iid = layer.fieldNameIndex("INUNDATED")
                    layer.startEditing()
                    for feat in it:
                        #if INUNDATED == 2
                        #QMessageBox.information(self, "Information", str(feat.attributes()[iid]))
                        if (feat.attributes()[iid] == 2):
                            layer.changeAttributeValue(feat.id(), fid, layerLabels[layerLabelCount])
                    layer.commitChanges()
                    layerLabelCount += 1  

                
                #merge layers that starts with "Estimated" 
                #save as sld filename
                #open layer        
                #for l in layers:
                layers = registry.mapLayers()        
                lm = []
                for name, layer in layers.iteritems():                            
                    ...
                    #if (name.find(" Estimated ") != -1):
                    #if (name.endswith("Impact")): 
                    if "Estimated" in name:                   
                        #QMessageBox.information(self, "Debug", name)                                          
                        legend.setLayerVisible(layer, False)
                        lm.append(layer.originalName())
                        #Debug
                        #QMessageBox.information(self, "Information", layer.originalName())

                #mmqgis_merge(qgis, layernames, savename, addlayer):
                path = os.getcwd() + "/out/Merged.shp" 
                mmqgisresult = mmqgis_library.mmqgis_merge(self.iface, lm, path, True)

                

                #finished
                QApplication.restoreOverrideCursor()
                QMessageBox.information(self, "Information", "Processing Complete.\r\nClick OK To Generate Consolidated Report.")    
                ...
                rno_medium_impact = 0
                rno_high_impact = 0
                rno_extreme_impact = 0

                layer = registry.mapLayersByName("Low Estimated Impact")[0]                
                rno_total = layer.featureCount()              
                rno_no_impact =  sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"INUNDATED" = 0')))
                rno_low_impact =  sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"INUNDATED" = 2')))
                
                layer = registry.mapLayersByName("Medium Estimated Impact")[0] 
                rno_medium_impact =  sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"INUNDATED" = 2')))

                layer = registry.mapLayersByName("High Estimated Impact")[0] 
                rno_high_impact = sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"INUNDATED" = 2')))     

                layer = registry.mapLayersByName("Extreme Estimated Impact")[0]  
                rno_extreme_impact = sum(1 for _ in layer.getFeatures(QgsFeatureRequest().setFilterExpression(u'"INUNDATED" = 2')))         


                #QMessageBox.information(self, "Debug", str(rno_extreme_impact))    


                report = m.Message()
                report.add(LOGO_ELEMENT)
                report.add(m.Heading(self.tr('PacSafe Consolidated Analysis Results'), **INFO_STYLE))
                report.add(m.Text("<b>Results Summary</b><br/><br/><table style='width: 100%;border-collapse: collapse;border: 1px solid black;'><tr style='background-color:lightgray;'><td style='width: 85%;'><b>Total Buildings</b></td><td><b>" + str(rno_total) + "</b></td></tr><tr><td>No Impact</td><td>" + str(rno_no_impact) + "</td></tr><tr><td>Low Impact</td><td>" + str(rno_low_impact) + "</td></tr><tr><td>Medium Impact</td><td>" + str(rno_medium_impact) + "</td></tr><tr><td>High</td><td>" + str(rno_high_impact) + "</td></tr><tr><td>Extreme Impact</td><td>" + str(rno_extreme_impact) + "</td></tr><table>").to_html())
                report.add(m.Text("<br/><b>Breakdown By Type</b><br/><i>Awaiting...</i>"))
            ...
            if (Config.get("settings", "multi") == "False"):
                #QMessageBox.information(self, "Debug", "MultiRun: " + str(Config.get("settings", "multi")))
                self.analysis.run_analysis()
