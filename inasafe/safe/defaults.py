...
def disclaimer():
    ...
    """
    text = tr(
        'PacSAFE has been developed from InaSAFE by the Secretariat of the Pacific Community in collaboration with the National Institute of Water & Atmospheric Research Ltd, with support from the Asian Development Bank. InaSAFE has been jointly developed by Indonesian Government-BNPB, Australian Government-AIFDR and the World Bank-GFDRR.'
        'These agencies and the individual software developers of InaSAFE and PacSAFE take no responsibility for the correctness of outputs from InaSAFE of PacSAFE or decisions derived as a consequence.')
    return text
...
def limitations():
    ...
    limitation_list.append(tr('PacSAFE and InaSAFE are not hazard modelling tools'))
    limitation_list.append(tr('PacSAFE has initially been developed from InaSAFE to access and use data developed as part of PCRAFI. Some functionality of InaSAFE has not yet been implemented. '))
    limitation_list.append(tr('InaSAFE and PacSAFE are Free and Open Source Software (FOSS) projects, published under the GPL V3 license. As such you may freely download, share and (if you like) modify the software.'))
