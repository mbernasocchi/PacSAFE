...
class FunctionOptionsDialog(QtGui.QDialog, FORM_CLASS):
    ...
    def __init__(self, parent=None):
        ...
        self.setupUi(self)
        self.setWindowTitle(self.tr('PacSafe impact function configuration'))
    ...
    def accept(self):
        ...
        try:
            ...
            #PacSafe
            multi = str(self.checkMulti.isChecked())
            low = self.textLow.text().strip()
            med = self.textMed.text().strip()
            high = self.textHigh.text().strip()
            ext = self.textExt.text().strip()
            #QMessageBox.information(self, "PacSafe", high)
            path = tempfile.tempdir + "/pacsafe.ini"
            cfgfile = open(path, 'w')
            Config = ConfigParser.ConfigParser()
            Config.add_section('settings')
            Config.set('settings','multi',self.checkMulti.isChecked())
            Config.set('settings','low', low)
            Config.set('settings','med', med)
            Config.set('settings','high', high)
            Config.set('settings','ext', ext)
            Config.write(cfgfile)
            cfgfile.close()

            QMessageBox.information(self, "PacSafe", "Functions Options Saved.")
            #m = QMessageBox(self)
            #m.setWindowTitle("PacSafe")
            #m.setText("Functions Options Saved.") 
